...
namespace c2x64 {
    ...
    Lexer::Lexer(string_view input) : input_(input) {}
    ...
    Token Lexer::lex_token() {
        ...
        if (is_ident_start(c)) {
            ...
            string word(input_.substr(begin, i_ - begin));
        ...
        if (isdigit(static_cast<unsigned char>(c))) {
            size_t begin = i_;
            while (!eof() && isdigit(static_cast<unsigned char>(current()))) advance();
            string digits(input_.substr(begin, i_ - begin));
